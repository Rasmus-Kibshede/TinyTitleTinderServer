...
export const responseError = (res: Response<unknown, Record<string, unknown>>, response: Result<ApiResponse, BaseError>) => {
    res.status(response.success ? 400 : Number(response.error.statusCode)).send(response.success ? { success: response.success, data: response.result.data } : { success: response.success, message: response.error.message });
};